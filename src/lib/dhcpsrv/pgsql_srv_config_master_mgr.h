...
#ifndef PGSQL_SRV_CONFIG_MASTER_MGR_H
#define PGSQL_SRV_CONFIG_MASTER_MGR_H
...
class PgSqlMasterConfigExchange;

/// @brief Exchange used to interact with the master server configuration table
/// from the database
class PgSqlSrvConfigMasterMgr : public SrvConfigMasterMgr {
public:
    ...
    explicit PgSqlSrvConfigMasterMgr(const DatabaseConnection::ParameterMap& parameters);

    /// @brief Destructor
    virtual ~PgSqlSrvConfigMasterMgr();
    ...
    /// @brief Returns the type of backend used, in this case "postgresql". Useful
...
private:
    ...
    boost::scoped_ptr<PgSqlMasterConfigExchange> exchange_;
};

}  // namespace dhcp
}  // namespace isc

#endif  // PGSQL_SRV_CONFIG_MASTER_MGR_H
