...
TEST_F(Dhcp6ParserTest, subnetGlobalDefaults) {
    ...
    EXPECT_EQ(2000U, subnet->getPreferred().getMin());
    EXPECT_EQ(4000U, subnet->getPreferred().getMax());
    EXPECT_EQ(4000U, subnet->getValid().get());
...
TEST_F(Dhcp6ParserTest, subnetLocal) {
    ...
    EXPECT_EQ(2U, subnet->getPreferred().getMin());
    EXPECT_EQ(4U, subnet->getPreferred().getMax());
...
TEST_F(Dhcp6ParserTest, pdPoolPrefixExclude) {
    ...
    EXPECT_EQ(72, static_cast<int>(pd_exclude_option->getExcludedPrefixLength()));
}
...
TEST_F(Dhcp6ParserTest, pdPoolList) {
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        EXPECT_EQ((80U + (i * 8)), p6->getLength());
    }
}
...
TEST_F(Dhcp6ParserTest, optionDataTwoSpaces) {
    ...
    EXPECT_EQ(38U, desc1.option_->getType());
...
TEST_F(Dhcp6ParserTest, optionDataEncapsulate) {
    ...
    EXPECT_EQ(100U, desc.option_->getType());
    ...
    EXPECT_EQ(110U, option_foo->getType());
...
TEST_F(Dhcp6ParserTest, optionDataInMultipleSubnets) {
    ...
    ConstSubnet6Ptr subnet1 = CfgMgr::instance().getStagingCfg()->
        getCfgSubnets6()->selectSubnet(IOAddress("2001:db8:1::5"), classify_);
    ASSERT_TRUE(subnet1);
    ...
    ConstSubnet6Ptr subnet2 = CfgMgr::instance().getStagingCfg()->
...
TEST_F(Dhcp6ParserTest, stdOptionData) {
    ...
    EXPECT_EQ(12345U, optionIA->getIAID());
    EXPECT_EQ(6789U, optionIA->getT1());
    EXPECT_EQ(1516U, optionIA->getT2());
}
...
TEST_F(Dhcp6ParserTest, rdnssOption) {
    ...
    EXPECT_EQ(3U, optionCustom->readInteger<uint8_t>(1));
...
TEST_F(Dhcp6ParserTest, subnetRelayInfoList) {
    ...
    ConstSubnet6Ptr subnet = CfgMgr::instance().getStagingCfg()->
        getCfgSubnets6()->selectSubnet(IOAddress("2001:db9::abcd"),
                                       classify_, true);
    ASSERT_TRUE(subnet);
...
TEST_F(Dhcp6ParserTest, reservationWithOptionDefinition) {
    ...
    EXPECT_EQ(11U, opt_foo->getValue());
}
...
TEST_F(Dhcp6ParserTest, comments) {
    ...
    EXPECT_EQ(100U, host->getIPv6SubnetID());
...
TEST_F(Dhcp6ParserTest, clientClassValidPreferredLifetime) {
    ...
    EXPECT_EQ(class_def->getPreferred().getMin(), 4000U);
    EXPECT_EQ(class_def->getPreferred().get(), 5000U);
    EXPECT_EQ(class_def->getPreferred().getMax(), 6000U);
...
TEST_F(Dhcp6ParserTest, deprecatedRequireClientClassesCheck) {
    ...
    EXPECT_EQ(1U, pd_pool_class_list.size());
...
TEST_F(Dhcp6ParserTest, deprecatedClientClassesCheck) {
    ...
    ASSERT_EQ(1U, pd_pool_class_list.size());
    cclasses = pd_pool_class_list.begin();
    EXPECT_EQ(*cclasses, "four");
...
TEST_F(Dhcp6ParserTest, hostsDatabase) {
    string config = R"(
    {
        "hosts-database": {
            "type": "mysql"
    ...
    configure(config, CONTROL_RESULT_ERROR, expected);
    EXPECT_EQ(1U, countFile("DHCP6_CONFIG_HOSTS_DATABASE_DEPRECATED"));
}
