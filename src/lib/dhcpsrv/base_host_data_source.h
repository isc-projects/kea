...
public:
 ...
 0;
    ...
    ///
    /// This method updates the frequently changing information about the
    /// host. Such information is typically generated by the DHCP server or
    /// a hooks library while processing the client's packet. Currently
    /// supported runtime information are: authentication key (used for
    /// triggering client's reconfiguration) and user context (also used
    /// for authentication and for hook specific purposes).
    ///
    /// Using this function avoids updating the entire host in the database,
    /// which may be an expensive operation, becase the host information
    /// can be spread across multiple SQL tables. This function merely
    /// updates the data in the selected columns.
    ///
    /// @param host Pointer to the @c Host object including the modified
    /// runtime information.
    /// @return Number of updated host instances.
    virtual size_t updateRuntimeInfo(const HostPtr& host) = 0;
