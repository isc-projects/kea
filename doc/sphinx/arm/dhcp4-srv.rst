...
-  ``-F`` - As of Kea 3.3.0, makes the server exit immediately on any fatal
   error detected when executing API commands. Default behavior (parameter not set)
...
.. note::
   ...
   ``/etc/systemd/system/isc-kea-dhcp4-server.service.d/``
   (or for RPM packages ``/etc/systemd/system/kea-dhcp4.service.d/``) directory
