...
public:
    ...
    AcceptFilter6TestClient(IOServicePtr& io_service,
    ...
    /// @brief Destructor.
    virtual ~AcceptFilter6TestClient() {
        close();
    }
...
/// @brief Type of pointers to AcceptFilter6TestClient objects.
typedef boost::shared_ptr<AcceptFilter6TestClient> AcceptFilter6TestClientPtr;

/// @brief Test fixture class for testing BulkLeaseQueryService::acceptFilter.
class AcceptFilter6Test : public ::testing::Test {
public:
    ...
    void configBulkLeaseQuery(string config) {
        ...
        ASSERT_NO_THROW_LOG(impl_.reset(new LeaseQueryImpl6(cfg)));
    ...
    /// @param port port number of the server.
    void createClient(const std::string& address = "::1",
                      uint16_t port = 18123) {
        AcceptFilter6TestClientPtr client;
        ASSERT_NO_THROW(client.reset(new AcceptFilter6TestClient(io_service_,
                                                                 address,
                                                                 port)));
    ...
    /// @brief DHCPv6 lease query implementation.
    LeaseQueryImplPtr impl_;

    /// @brief List of test clients.
    list<AcceptFilter6TestClientPtr> clients_;
...
TEST_F(AcceptFilter6Test, basic) {
    ...
    endpoint.address(ip::make_address("::1"));
...
TEST_F(AcceptFilter6Test, twoRequesters) {
    // Multi-threading is required, limit to 1 to avoid races.
    configMultiThreading(true, 1);
...
/// @brief Verify that acceptFilter with more than one configured requesters
/// works as expected, i.e. the requester can't eat all connections.
TEST_F(AcceptFilter6Test, differentRequesters) {
    // Multi-threading is required, limit to 1 to avoid races.
    configMultiThreading(true, 1);
    ...
    ip::tcp::endpoint endpoint(ip::make_address("2001:db8::1"), 18123);
    EXPECT_TRUE(BulkLeaseQueryService::acceptFilter(endpoint));
...
TEST_F(AcceptFilter6Test, oneConnection) {
    // Multi-threading is required, limit to 1 to avoid races.
    configMultiThreading(true, 1);
