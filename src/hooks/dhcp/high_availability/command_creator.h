...
private:
    ...
    /// DHCPv4 or DHCPv6 server via control socket. The control socket
    /// does not requires a "service" parameter which provides the list
    /// of servers to which the command should be forwarded. In our case,
    /// we always send commands to a single server so this method appends
    /// a single element list to the command.
    ///
