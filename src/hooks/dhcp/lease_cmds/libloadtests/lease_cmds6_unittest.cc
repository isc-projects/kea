...
public:
    ...
    ///
    /// @param line_no source line function was invoked.
    ...
    void checkLease6(int line_no, isc::data::ConstElementPtr l, std::string ip,
                     uint8_t prefixlen, uint32_t subnet_id, std::string duid,
                     std::string hwaddr = std::string(), uint32_t pool_id = 0) {
        std::stringstream ss;
        ss << "checkLease6 called at line:" << line_no << std::endl;
        SCOPED_TRACE(ss.str());
        ASSERT_TRUE(l);
    ...
    /// @param assigned_nas The expected value of assigned nas in subnet.
    /// @param declined The expected value of declined addresses in subnet.
    /// @param assigned_pds The expected value of assigned pds in subnet.
    /// @param registered_nas The expected value of registered nas in subnet.
    void checkLease6Stats(int line_no, isc::dhcp::SubnetID id, int32_t assigned_nas,
                          int32_t declined, int32_t assigned_pds,
                          int32_t registered_nas) {
        std::stringstream ss;
        ss << "checkLease6Stats called at line:" << line_no << std::endl;
        SCOPED_TRACE(ss.str());
    ...
    /// @brief Check that a simple, well formed registered lease6 can be added.
    void testLease6AddRegisteredLeases();
    ...
    /// @brief Check that lease6-get-by-state can handle a situation when
    /// the query is broken (required parameter is missing).
    void testLease6GetByStateParams();

    /// @brief Check that lease6-get-by-state works as expected (find no
    /// lease).
    void testLease6GetByStateFind0();

    /// @brief Check that lease6-get-by-state works as expected (find two
    /// leases).
    ...
    /// @brief Check that a registered lease6 can't be updated to another state.
    void testLease6UpdateFromRegistered();

    /// @brief Check that a not registered lease6 can't be updated to registered.
    void testLease6UpdateToRegistered();

    /// @brief Check that a registered lease6 can be updated. We're changing
    /// nearly everything at the exception of course the address and state.
    void testLease6UpdateRegistered();
    ...
    /// @brief Check that lease6-del(subnet-id, addr6) can delete a registered
    /// lease.
    void testLease6DelByAddrRegistered();
...
void Lease6CmdsTest::testLease6Add() {
    ...
    checkLease6Stats(__LINE__, 0, 0, 0, 0, 0);

    checkLease6Stats(__LINE__, 66, 0, 0, 0, 0);

    checkLease6Stats(__LINE__, 99, 0, 0, 0, 0);
    ...
    checkLease6Stats(__LINE__, 0, 1, 0, 0, 0);

    checkLease6Stats(__LINE__, 66, 1, 0, 0, 0);
...
void Lease6CmdsTest::testLease6AddDeclinedLeases() {
    ...
    checkLease6Stats(__LINE__, 0, 1, 1, 0, 0);

    checkLease6Stats(__LINE__, 66, 1, 1, 0, 0);
...
void Lease6CmdsTest::testLease6AddRegisteredLeases() {
    ...
    checkLease6Stats(__LINE__, 66, 0, 0, 0, 1);
...
void Lease6CmdsTest::testLease6AddExisting() {
    ...
    checkLease6Stats(__LINE__, 0, 4, 0, 0, 0);

    checkLease6Stats(__LINE__, 66, 2, 0, 0, 0);

    checkLease6Stats(__LINE__, 99, 2, 0, 0, 0);
...
void Lease6CmdsTest::testLease6AddPrefix() {
    ...
    checkLease6Stats(__LINE__, 0, 0, 0, 1, 0);

    checkLease6Stats(__LINE__, 66, 0, 0, 1, 0);
...
void Lease6CmdsTest::testLease6GetByAddrPrefix() {
    ...
    // Now check that the lease was indeed returned.
    checkLease6(__LINE__, lease, "2001:db8:1234:ab::", 56, 66, "77:77:77:77:77:77:77:77");
}
...
void Lease6CmdsTest::testLease6GetAll() {
    ...
    checkLease6(__LINE__, leases, "2001:db8:1::1", 0, 66, "42:42:42:42:42:42:42:42");
    checkLease6(__LINE__, leases, "2001:db8:1::2", 0, 66, "56:56:56:56:56:56:56:56", "", 5);
    checkLease6(__LINE__, leases, "2001:db8:2::1", 0, 99, "42:42:42:42:42:42:42:42");
    checkLease6(__LINE__, leases, "2001:db8:2::2", 0, 99, "56:56:56:56:56:56:56:56");
}
    ...
    for (auto i = 0; i < 3; ++i) {
        ...
        if (!leases->empty()) {
            ...
            for (auto const& lease : leases->listValue()) {
                ...
                checkLease6(__LINE__, leases, last_address, 0, from_mgr->subnet_id_,
...
void Lease6CmdsTest::testLease6GetByHwAddressFind2() {
    ...
    ASSERT_TRUE(lease);
    checkLease6(__LINE__, lease, "2001:db8:2::1", 0, 99, "42:42:42:42:42:42:42:42", "08:08:08:08:08:08");
    lease = leases->get(1);
    ASSERT_TRUE(lease);
    checkLease6(__LINE__, lease, "2001:db8:1::1", 0, 66, "42:42:42:42:42:42:42:42", "08:08:08:08:08:08");
}
...
void Lease6CmdsTest::testLease6GetByStateParams() {
    // No parameters whatsoever.
    string cmd =
        "{\n"
...
void Lease6CmdsTest::testLease6GetByStateFind0() {
    ...
    string cmd =
        ...
        "    }\n"
        "}";
    string exp_rsp = "0 IPv6 lease(s) found with state released (3) in subnet 1.";
    testCommand(cmd, CONTROL_RESULT_EMPTY, exp_rsp);
}

void Lease6CmdsTest::testLease6GetByStateFind2() {
    // Initialize lease manager (true = v6, true = add leases, true = declined)
    ...
    string cmd =
        ...
        "    }\n"
        "}";
    string exp_rsp = "4 IPv6 lease(s) found with state declined (1).";
    ConstElementPtr rsp = testCommand(cmd, CONTROL_RESULT_SUCCESS, exp_rsp);
...
void Lease6CmdsTest::testLease6UpdateDeclinedLeases() {
    ...
    checkLease6Stats(__LINE__, 0, 4, 4, 0, 0);

    checkLease6Stats(__LINE__, 66, 2, 2, 0, 0);

    checkLease6Stats(__LINE__, 99, 2, 2, 0, 0);
    ...
    checkLease6Stats(__LINE__, 0, 4, 3, 0, 0);

    checkLease6Stats(__LINE__, 66, 2, 1, 0, 0);
...
void Lease6CmdsTest::testLease6UpdateFromRegistered() {
    // Initialize lease manager (true = v6, false = don't add leases)
    initLeaseMgr(true, false);

    // We need a registered lease. We need to create it by hand.
    // Let's start with regular lease and make it a registered lease.
    Lease6Ptr l = createLease6("2001:db8:1::1", 66, 0x77);
    l->state_ = 4;
    lmptr_->addLease(l);
    ...
    string txt =
        ...
        "    }\n"
        "}";
    string exp_rsp = "illegal reuse of registered address 2001:db8:1::1";
    testCommand(txt, CONTROL_RESULT_ERROR, exp_rsp);
}

void Lease6CmdsTest::testLease6UpdateToRegistered() {
    // Initialize lease manager (true = v6, true = add leases)
    initLeaseMgr(true, true);
    ...
    string txt =
        ...
        "    }\n"
        "}";
    string exp_rsp = "address in use: 2001:db8:1::1 can't be registered";
    testCommand(txt, CONTROL_RESULT_ERROR, exp_rsp);
}

void Lease6CmdsTest::testLease6UpdateRegistered() {
    // At the difference of assigned addresses which come from reservations
    // and pools, registered addresses can change subnet. So here the code
    // was taken from initLeaseMgr and uses 2 subnets with overlapping
    // address ranges.
    ...
    CfgMgr& cfg_mgr = isc::dhcp::CfgMgr::instance();
    Subnet6Ptr subnet66(new Subnet6(IOAddress("2001:db8:1::"), 48, 1, 2, 3, 4, 66));
    Subnet6Ptr subnet99(new Subnet6(IOAddress("2001:db8:1::"), 56, 1, 2, 3, 4, 99));
    ...
    checkLease6Stats(__LINE__, 99, 0, 0, 0, 1);

    // Now check that the lease is still there.
    l = lmptr_->getLease6(Lease::TYPE_NA, IOAddress("2001:db8:1::1"));
    ASSERT_TRUE(l);
...
void Lease6CmdsTest::testLease6DelByAddr() {
    ...
    checkLease6Stats(__LINE__, 0, 3, 0, 0, 0);
...
void Lease6CmdsTest::testLease6DelByAddrDeclinedLeases() {
    ...
    checkLease6Stats(__LINE__, 0, 3, 3, 0, 0);
...
void Lease6CmdsTest::testLease6DelByAddrRegistered() {
    // Initialize lease manager (true = v6, false = don't add leases)
    initLeaseMgr(true, false);
...
void Lease6CmdsTest::testLease6Wipe() {
    ...
    checkLease6Stats(__LINE__, 0, 2, 0, 0, 0);
...
void Lease6CmdsTest::testLease6BulkApply() {
    ...
    checkLease6Stats(__LINE__, 99, 3, 0, 0, 0);
...
void Lease6CmdsTest::testLease6BulkApplyAddsOnly() {
    ...
    checkLease6Stats(__LINE__, 99, 1, 0, 0, 0);
...
TEST_F(Lease6CmdsTest, lease6AddRegisteredLeases) {
    testLease6AddRegisteredLeases();
}

TEST_F(Lease6CmdsTest, lease6AddRegisteredLeasesMultiThreading) {
    MultiThreadingTest mt(true);
    testLease6AddRegisteredLeases();
}
...
TEST_F(Lease6CmdsTest, lease6GetByStateParams) {
    testLease6GetByStateParams();
}

TEST_F(Lease6CmdsTest, lease6GetByStateParamsMultiThreading) {
    MultiThreadingTest mt(true);
    testLease6GetByStateParams();
}

TEST_F(Lease6CmdsTest, lease6GetByStateFind0) {
    testLease6GetByStateFind0();
}

TEST_F(Lease6CmdsTest, lease6GetByStateFind0MultiThreading) {
    MultiThreadingTest mt(true);
    testLease6GetByStateFind0();
}

TEST_F(Lease6CmdsTest, lease6GetByStateFind2) {
...
TEST_F(Lease6CmdsTest, lease6UpdateFromRegistered) {
    testLease6UpdateFromRegistered();
}

TEST_F(Lease6CmdsTest, lease6UpdateFromRegisteredMultiThreading) {
    MultiThreadingTest mt(true);
    testLease6UpdateFromRegistered();
}

TEST_F(Lease6CmdsTest, lease6UpdateToRegistered) {
    testLease6UpdateToRegistered();
}

TEST_F(Lease6CmdsTest, lease6UpdateToRegisteredMultiThreading) {
    MultiThreadingTest mt(true);
    testLease6UpdateToRegistered();
}

TEST_F(Lease6CmdsTest, lease6UpdateRegistered) {
    testLease6UpdateRegistered();
}

TEST_F(Lease6CmdsTest, lease6UpdateRegisteredMultiThreading) {
    MultiThreadingTest mt(true);
    testLease6UpdateRegistered();
}
...
TEST_F(Lease6CmdsTest, lease6DelByAddrRegistered) {
    testLease6DelByAddrRegistered();
}

TEST_F(Lease6CmdsTest, lease6DelByAddrRegisteredMultiThreading) {
    MultiThreadingTest mt(true);
    testLease6DelByAddrRegistered();
}
