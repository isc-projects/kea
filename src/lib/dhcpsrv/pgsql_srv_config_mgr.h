...
#ifndef PGSQL_SRV_CONFIG_MGR_H
#define PGSQL_SRV_CONFIG_MGR_H
...
class PgSqlSrvConfigMgr : public SrvConfigMgr {
public:
    /// @brief Constructor
    ///
    ...
    ///        failed.
    PgSqlSrvConfigMgr(const DatabaseConnection::ParameterMap& parameters);

    /// @brief Destructor (closes database)
    virtual ~PgSqlSrvConfigMgr();
    ...
    ///
    /// @param old_config_timestamp If there is already a server configuration in
    ///     the database then the configuration's timestamp from database is
...
private:
    ...
    SrvConfigInfoPtr getConfigTimestampCommon(const StatementIndex statement_index) const;
    ...
    SrvConfigInfoPtr getConfigCommon(const StatementIndex statement_index) const;
    ...
    ///     otherwise
    bool insertConfigCommon(const StatementIndex statement_index,
    ...
    ///     otherwise
    bool updateConfigCommon(const StatementIndex statement_index,
                            const std::string& config_id,
                            const std::string& json_data,
                            const std::string& generic_data) const;

    /// @brief PostgreSQL connection handle
    mutable PgSqlConnection conn_;
};

}  // namespace dhcp
}  // namespace isc

#endif  // PGSQL_SRV_CONFIG_MGR_H
