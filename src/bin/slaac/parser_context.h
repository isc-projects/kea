...
#ifndef SLAAC_PARSER_CONTEXT_H
#define SLAAC_PARSER_CONTEXT_H
#include <string>
#include <map>
#include <vector>
#include <slaac/slaac_parser.h>
#include <slaac/parser_context_decl.h>
#include <cc/dhcp_config_error.h>
#include <exceptions/exceptions.h>

// Tell Flex the lexer's prototype ...
#define YY_DECL isc::slaac::SlaacParser::symbol_type slaac_lex (ParserContext& driver)
...
///        Slaac-agent config file parser.
///
...
/// parse only content of the Slaac-agent object, which is a subset
...
public:
    ...
    ///
    /// SlaacParser is able to parse several types of scope. Usually,
    /// when it parses a config file, it expects the data to have a map
    /// with Slaac-agent in it and all the parameters within that map.
    ...
    typedef enum {
        ...
        /// This parser will expect the content as Slaac config wrapped
        /// in a map (that's the regular config file)
        PARSER_SLAAC,

        /// This parser will expect only the content of Slaac.
        PARSER_SUB_SLAAC
    } ParserType;
    ...
    ///
    /// @param str string to be parsed
    /// @param parser_type specifies expected content (usually SLAAC or generic JSON)
    ...
    ///
    /// @param filename file to be parsed
    /// @param parser_type specifies expected content (usually PARSER_SLAAC
    ///                                                or PARSER_JSON)
    /// @return Element structure representing parsed text.
    ...
    /// @throw ParseError
    void error(const isc::slaac::location& loc, const std::string& what);

    /// @brief Error handler
    ///
    /// This is a simplified error reporting tool for possible future
    /// cases when the SlaacParser is not able to handle the packet.
    ///
    /// @param what string explaining the nature of the error.
    /// @throw ParseError
    ...
    isc::data::Element::Position loc2pos(isc::slaac::location& loc);
    ...
    /// for logging errors.
    isc::slaac::location loc_;

    /// @brief Location stack
    std::vector<isc::slaac::location> locs_;
    ...
    /// does not parse properly. Slaac Agent parser uses simplified
