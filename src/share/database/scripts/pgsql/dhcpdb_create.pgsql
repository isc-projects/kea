...
CREATE TABLE IF NOT EXISTS flq_pool4 (
    ...
    last_returned_address BIGINT NOT NULL DEFAULT 0,
    subnet_id BIGINT NOT NULL,
...
LANGUAGE plpgsql AS $$
BEGIN
    IF (p_start_address = 0 OR p_start_address > p_end_address)
    THEN
        RAISE EXCEPTION 'Invalid range: % - %', ('0.0.0.0'::inet + p_start_address),
                        ('0.0.0.0'::inet + p_end_address);
    END IF;
    ...
    THEN
        ...
        INSERT INTO flq_pool4 (start_address, end_address, subnet_id, last_returned_address)
           VALUES (p_start_address, p_end_address, p_subnet_id, 0)
...
CREATE OR REPLACE FUNCTION sflqPickFreeLease4(p_start_address BIGINT,
                                              p_end_address BIGINT)
RETURNS BIGINT
AS $$
DECLARE
...
BEGIN
    ...
    SELECT id, last_returned_address INTO pool_id, last_address
        FROM flq_pool4
        WHERE start_address = p_start_address
        AND end_address = p_end_address;
    ...
    IF (last_address < p_start_address or last_address >= p_end_address)
    THEN
        -- Set last pick to start address less one.
       last_address = p_start_address - 1;
    END IF;
...
BEGIN
    INSERT INTO lease4 (address, hwaddr, client_id, valid_lifetime, expire,
         ...
         VALUES ( p_address, p_hwaddr, p_client_id, p_valid_lifetime, p_expire,
    ...
    THEN
        -- Update SFLQ data.
        -- If it is expired-reclaimed delete add it to the
...
BEGIN
    INSERT INTO lease6 (address, duid, valid_lifetime, expire, subnet_id,
         ...
         VALUES (p_address, p_duid, p_valid_lifetime, p_expire, p_subnet_id,
                 p_pref_lifetime, p_lease_type, p_iaid, p_prefix_len, p_fqdn_fwd,
...
-- or the foreign key constraint will blow this up.
UPDATE lease6 SET lease_type = 0 WHERE lease_type = 1;
UPDATE flq_pool6 SET lease_type = 0 WHERE lease_type = 1;
DELETE FROM lease6_stat_by_client_class WHERE lease_type = 1;
DELETE FROM lease6_pool_stat WHERE lease_type = 1;
