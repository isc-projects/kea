...
void IfaceMgr::detectIfaces(bool update_only) {
    ...
    for (auto const& msg : link_info) {
        ...
        if ((interface_info->ifi_flags & IFF_BROADCAST) && attribs_table[IFA_BROADCAST]) {
            iface->setBcastMac(static_cast<const uint8_t*>(RTA_DATA(attribs_table[IFA_BROADCAST])),
                               RTA_PAYLOAD(attribs_table[IFA_BROADCAST]));
